...
generate_list() {
        ...
        for region in ${region_list}; do
                ...
                data=`aws ec2 describe-network-interfaces --query 'NetworkInterfaces[*].[Status,Description,NetworkInterfaceId,PrivateIpAddresses[*].PrivateIpAddress,Groups[*].GroupName,AvailabilityZone,Attachment.InstanceId,PrivateIpAddresses[*].Association.PublicIp,OwnerId]' --output json  --region $region`
        ...
        #replace owner-id with account name
        sed -i "s/$2/$1/g" $data_file.new

        #replace nulls with a null json data
        sed -i 's/null/""/g' $data_file.new
}
